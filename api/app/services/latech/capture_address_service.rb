...
module ::Latech
  class CaptureAddressService < ApplicationService
    def call
      if cached_address
        Latech::HandleMakeSureAssignmentWorker.perform_async(
          params.merge!(
            address_id: cached_address[:id]
          ).stringify_keys!
        )
      else
    ...
    def cached_address
      handle_response[:content][:payload] = Rails.cache.fetch(
        "#{__method__}/#{params.fetch(:zip)}",
        expire_in: 12.hours,
        skip_nil: true
      ) do
