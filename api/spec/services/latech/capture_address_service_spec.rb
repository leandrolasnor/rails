...
RSpec.describe Latech::CaptureAddressService, type: :service do
  ...
  context 'when there is not cached address' do
    ...
    context 'and a error was raised on call to HandleCaptureAddressWorker' do
      ...
      before do
        allow(Rails.cache).
          to receive(:fetch).
          with(cache_key_address, expire_in: 12.hours, skip_nil: true).
          and_return(nil)
  ...
  context 'when there is cached address' do
    ...
    context 'and HandleMakeSureAssignmentWorker is called' do
      before do
        ...
        allow(Rails.cache).
          to receive(:fetch).
          with(cache_key_address, expire_in: 12.hours, skip_nil: true).
          and_return(cached_address)
        allow(Latech::HandleMakeSureAssignmentWorker).
          to receive(:perform_async).
          with(params.merge!(address_id: cached_address[:id]))
      end

      it 'must to return successful body content' do
        expect(service).to eq successful_response
        expect(Latech::HandleMakeSureAssignmentWorker).
    ...
    context 'on a error was raised on call to HandleMakeSureAssignmentWorker' do
      ...
      before do
        ...
        allow(Latech::HandleMakeSureAssignmentWorker).
          to receive(:perform_async).
          with(params.merge!(address_id: cached_address[:id])).and_raise(error)
      end
