...
RSpec.describe HandleShowAlbumWorker, type: :worker do
  ...
  context "when there is a success case" do
    ...
    before do
      allow(Albums).to receive(:show).with(params).and_yield("album", nil)
  ...
  context "when there is a failure case" do
    let(:event_failure) { {type: 'ERRORS_FROM_ALBUM_FETCHED', payload:{errors: ["Couldn't find Album with 'id'=9999"]}} }
    before do
      allow_any_instance_of(Album).to receive(:artist).and_return({})
      allow(Albums).to receive(:show).with(params).and_yield(nil, ["Couldn't find Album with 'id'=9999"])
