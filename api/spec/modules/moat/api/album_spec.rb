...
RSpec.describe Moat::Api, type: :module do
  subject(:moat) { described_class::Album.new(album) }
  let(:call) { moat.artist }
  let(:album) { create(:album) }
  let(:moat_url) { "#{ENV['MOAT_URI']}?artist_id=#{album.artist_id}" }
  let(:moat_headers) { {"Basic"  => Rails.application.credentials.dig(:moat, :token)} }

  context "#artist bring a response code is not equal 200" do
    ...
    before do
      ...
      allow(JSON).to receive(:parse).and_return double(first: moat_response_body)
      call
    end
