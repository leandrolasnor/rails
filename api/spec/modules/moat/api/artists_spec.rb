...
RSpec.describe Moat::Api, type: :module do
  subject(:moat) { described_class::Artist }
  let(:call) { moat.all }
  let(:moat_url) { "#{ENV['MOAT_URI']}" }
  let(:moat_headers) { {"Basic"  => Rails.application.credentials.dig(:moat, :token)} }

  context "#all bring a response code equal 200" do
    let(:moat_response) { instance_double(HTTParty::Response, body: moat_response_body, code: 200) }
    let(:moat_response_body) {
      [
    ...
    before do
      ...
      allow(JSON).to receive(:parse).and_return double(flatten: moat_response_body)
      call
    end
  ...
  context "#all bring a response code is not equal 200" do
